...
export default function ProfessionalDashboard() {
        ...
        if (ordersResponse.success) {
          ...
          setError((prev) =>
            prev
              ...
              : "Could not fetch order requests.",
          );
        }
      } catch (err) {
  ...
  const getJobsByStatus = (status: string): JobRequest[] => {
    ...
    switch (status) {
      case "new":
        return jobRequests
          .filter((job) =>
            ["new", "request"].includes(job.status.toLowerCase()),
          )
          .sort(sortByDateDesc);
      case "draft":
        return jobRequests
      ...
      case "bid":
        return jobRequests
          .filter((job) =>
            ["bid", "quotation", "submitted"].includes(
      ...
      case "active":
        return jobRequests
          .filter((job) =>
            ["assigned", "active"].includes(job.status.toLowerCase()),
          )
          .sort(sortByDateDesc);
      case "complete":
        return jobRequests
          .filter((job) =>
            ["completed", "complete", "past"].includes(
              job.status.toLowerCase(),
            ),
          )
          .sort(sortByDateDesc);
    ...
    switch (status) {
      case "new":
        return orderRequests
          .filter((order) =>
            ["PENDING", "NEW"].includes(order.status?.toUpperCase()),
          )
          .sort(sortByDateDesc);
      case "active":
        return orderRequests
          .filter((order) =>
            ["PROCESSING", "SHIPPED"].includes(order.status?.toUpperCase()),
          )
          .sort(sortByDateDesc);
      case "complete":
        return orderRequests
          .filter((order) =>
            ["DELIVERED", "COMPLETED", "PAST"].includes(
      ...
      case "bid":
        return orderRequests
          .filter((order) =>
            ["BIDDING", "BID"].includes(order.status?.toUpperCase()),
          )
          .sort(sortByDateDesc);
      default:
        return [];
          ...
          jobsToDisplay.map((job) => (
            <JobOrderCard
              ...
              onViewDetail={(selectedJob) =>
                navigate(`/professional/${activeSection}/job/${selectedJob.id}`)
              }
            />
          ))
        ) : (
  ...
  const renderOrdersLayout = (orders: OrderRequest[]) => {
    ...
    const ordersToDisplay = showAllOrders
      ? filteredOrders
        ...
        {ordersToDisplay.length > 0 ? (
          ordersToDisplay.map((order) => (
            <JobOrderCard
              ...
              getStatusColor={getStatusColor}
              getStatusIcon={getStatusIcon}
              onViewDetail={(selectedOrder) =>
                navigate(
                  `/professional/${activeSection}/order/${selectedOrder.id}`,
                )
              }
            />
          ))
  ...
  const renderSectionContent = () => {
    if (isLoading) {
      return (
        <div className="flex justify-center items-center p-16">
          <Loader className="animate-spin h-8 w-8 text-[#00007a]" />
        </div>
      );
    }
    if (error) {
    ...
    if (activeSection === "bid") {
      ...
      switch (activeSection) {
        case "new":
          sectionDescription =
            "New job opportunities and pending material orders";
          break;
        case "active":
          sectionDescription = "Currently ongoing work";
          break;
        case "complete":
          sectionDescription = "Completed work history and delivered orders";
          break;
        case "draft":
          sectionDescription = "Draft jobs and orders you are working on";
          break;
        default:
          sectionDescription = "All jobs and orders";
      }
    }
